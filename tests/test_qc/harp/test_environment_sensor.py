...
class MockHarpDevice:
    ...
    def _create_default_registers(self):
        if not self._registers:
            ...
            np.random.seed(0)
            temp = np.random.uniform(22, 28, len(time_index))
            humidity = np.random.uniform(40, 60, len(time_index))
...
def mock_env_sensor_device_bad_temp():
    ...
    temp = np.concatenate(
        [
            np.random.uniform(10, 15, 500),  # too low
            np.random.uniform(35, 40, 500),  # too high
        ]
    )
...
def mock_env_sensor_device_bad_humidity():
    ...
    time_index = np.linspace(0, 10, 1000)
    temp = np.random.uniform(22, 28, 1000)
    humidity = np.concatenate(
        [
            np.random.uniform(10, 20, 500),  # too low
            np.random.uniform(80, 90, 500),  # too high
        ]
    )
    sensor_df = pd.DataFrame(
        {"Temperature": temp, "Humidity": humidity, "MessageType": ["EVENT"] * 1000},
